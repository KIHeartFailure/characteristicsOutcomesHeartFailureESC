table1 <- function(data, name, subset, stratvar, vars, dec = 1) {
  ...
  for (k in seq_along(vars)) {
    ...
    if (k == 1) {
      deskout <- rbind(c("n", ns, rep(NA, 5)), tmp_deskout)
    } else {
...
catfunc <- function(var, stratvar, data, dec, nmiss) {
    ...
    for (j in seq_along(levvar)) {
      tmpdata <- data %>%
        mutate(tmpvar = case_when(
          !!sym(var) == levvar[j] ~ levvar[j],
          TRUE ~ "X"
        ))
      plevs[j] <- dF(chisq.test(table(
        tmpdata %>% pull(!!sym(stratvar)),
        tmpdata %>% pull(tmpvar)
      ))$p.value, dig = 3, p = TRUE)
  ...
  if (length(levs) > 2) {
    ...
    for (i in seq_along(levs)) {
      ...
      pstrat[1, i] <- dF(chisq.test(table(
        tmpdata %>% pull(!!sym(stratvar)),
        tmpdata %>% pull(!!sym(var))
      ))$p.value, dig = 3, p = TRUE)
      pnames[i] <- paste0("p ", paste(levs[seq_along(levs) != i], collapse = " vs "))
      ...
      if (length(levvar) > 2) {
        for (j in seq_along(levvar)) {
          ...
          pstrat[j + 1, i] <- dF(chisq.test(table(
            tmpdata %>% pull(!!sym(stratvar)),
            tmpdata %>% pull(tmpvar)
          ))$p.value, dig = 3, p = TRUE)
        }
